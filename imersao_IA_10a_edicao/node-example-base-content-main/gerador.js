...
const responseSchema = {
    ...
    items: {
        type: "OBJECT",
        properties: {
            "name": { "type": "STRING", "description": "Nome da videogame (ex: Resident Evil 4, Squad)." },
            "description": { "type": "STRING", "description": "Descrição concisa do videogame." },
            "year": { "type": "STRING", "description": "Ano de criação/lançamento (ex: '2013')." },
            "link": { "type": "STRING", "description": "URL oficial ou de documentação principal." },
            "tags": {
                "type": "ARRAY",
                "description": "Array de 3 a 5 strings que categorizam o jogo (ex: 'fps', 'mundo aberto', 'estrategia', 'simulador').",
                "items": { "type": "STRING" }
            }
        },
        "required": ["name", "description", "year", "link", "tags"]
    }
};
...
async function generateNewKnowledge(existingKnowledge) {
    ...
    const systemPrompt = `Você é um especialista em videogames. Sua tarefa é criar ${TOTAL_ITEMS} novas entradas sobre diferentes gêneros (terror, ação, mundo aberto, milsim, coop, etc.) com a mesma estrutura JSON. Garanta que cada entrada seja única e relevante. O foco é em jogos diversos e aclamados pelo público.`;
    
    // NOVO userQuery: Focado em 25 itens e evitando nomes existentes
    const userQuery = `Gere uma lista de ${TOTAL_ITEMS} novos jogos. Siga estritamente a estrutura JSON e o requisito de ser um ARRAY com EXATAMENTE ${TOTAL_ITEMS} objetos. Não inclua os 7 jogos iniciais (Minecraft, War Thunder, The Witcher 3: Wild Hunt, Grand Theft Auto V, League of Legends, Counter-Strike 2, Valorant) e NÃO use NENHUM dos seguintes nomes: ${existingNames}.`;
